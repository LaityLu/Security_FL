...
from .utils.uniform_sampler import (
    DistributedUniformWithReplacementSampler,
    UniformWithReplacementSampler, NonUniformWithReplacementSampler,
)
...
class PersonalizedDPDataLoader(DataLoader):
    def __init__(
            self,
            dataset: Dataset,
            *,
        ...
        """
        Args:
            dataset: class:`torch.utils.data.DataLoader`
            sample_rate: record sampling rates
            collate_fn: class:`torch.utils.data.DataLoader`
            drop_last: class:`torch.utils.data.DataLoader`
            generator: Random number generator used to sample elements
        """
        ...
        batch_sampler = NonUniformWithReplacementSampler(
            num_samples=len(dataset),  # type: ignore[assignment, arg-type]
            sample_rate=sample_rate,
            generator=generator,
            steps=steps
        ...
        if drop_last:
            logger.warning(
                "Ignoring drop_last as it is not compatible with PersonalizedDPDataLoader."
            )
